...
def load_tokenizer(filename='saved_models/tokenizer.json'):
    filename = os.path.join(BASE_DIR,f'api/services/recognizer/{filename}')
...
def load_model():
    ...
    model = tf.keras.models.load_model(
        os.path.join(BASE_DIR, 'api/services/recognizer/saved_models/model_fio_classificaion.h5'))
...
def load_xgb():
    ...
    with open(os.path.join(BASE_DIR, 'api/services/recognizer/saved_models/xgb.pickle.dat'), 'rb') as filehandler:
...
class Predictor():
    ...
    def __init__(self):
        self.directory = os.path.join(MEDIA_ROOT, 'output_files')
